cc.Class({
    ...
    onLoad: function () {
        ...
        this.qustion_tip_com = this.question_node.getChildByName("same_tip").getComponent(cc.Sprite);
        ...
        this.anim_tip_set = [];
        url = cc.url.raw("resources/game_scene/rect.png");
        sf = new cc.SpriteFrame(url);
        this.anim_tip_set.push(sf);
        url = cc.url.raw("resources/game_scene/tran.png");
        sf = new cc.SpriteFrame(url);
        this.anim_tip_set.push(sf);
        url = cc.url.raw("resources/game_scene/circle.png");
        sf = new cc.SpriteFrame(url);
        this.anim_tip_set.push(sf);
    },
    
    get_anim_set: function() {
    ...
    gen_anim_when_start: function(anim_array) {
        var time = 0.1;
        var delta = 0.3;
    ...
    gen_level0_map_data: function() {
        ...
        var level_anim1 = [1, 2, 3, 4, 5, 6, 7];
        var level_anim2 = [ 8, 9, 10, 11, 12, 13, 14];
        var level_anim3 = [15, 16, 17, 18, 19, 20, 21];
        
        
        var leve_random = [level_anim2, level_anim3, level_anim1];
        ...
        if (leve_random[0] == level_anim1) {
            this.qustion_tip_com.spriteFrame = this.anim_tip_set[0];    
        }
        else if(leve_random[0] == level_anim2) {
            this.qustion_tip_com.spriteFrame = this.anim_tip_set[1];
        }
        else if(leve_random[0] == level_anim3) {
            this.qustion_tip_com.spriteFrame = this.anim_tip_set[2];
        }
        
        
        
        var left_anim = [];
        for(var w = 0; w < leve_random[1].length; w ++) {
            left_anim.push(leve_random[1][w]);
        }
        
        for(var w = 0; w < leve_random[2].length; w ++) {
            left_anim.push(leve_random[2][w]);
        }
        
        left_anim.sort(function() {
            return Math.random() - 0.5;
        });
        
        var anim_array = [leve_random[0][0], left_anim[0], left_anim[1], left_anim[2], left_anim[3]];
        anim_array.sort(function() {
            return Math.random() - 0.5;
        });
        
        this.min_anim_type = leve_random[0][0];
        this.max_anim_type = left_anim[3];
    ...
    gen_game_data: function(anim_data) {
        var flip_block_comp;
    ...
    play_choose_succes_anim: function(card_value) {
    ...
    play_choose_error_anim: function(block) {
