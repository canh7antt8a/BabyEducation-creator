cc.Class({
    ...
    on_game_start: function() {
        ...
        for(var index = 0; index < 4; index ++) {
            var lhs = 1 + Math.random() * 9;
            lhs= Math.floor(lhs);
            
            var rhs = 1 + Math.random() * 9;
            rhs= Math.floor(rhs);
            
            rand_type.push([lhs, rhs]);
        }

        this.checkout_root.active = false;
        
        this.game_start = true;
        this.value_array = [[rand_type[0][0], rand_type[0][1], 0], [rand_type[1][0], rand_type[1][1], 0], [rand_type[2][0], rand_type[2][1], 0], [rand_type[3][0], rand_type[3][1], 0], 
                            [rand_type[3][0], rand_type[3][1], 1], [rand_type[2][0], rand_type[2][1], 1], [rand_type[1][0], rand_type[1][1], 1], [rand_type[0][0], rand_type[0][1], 1]];
    ...
    on_card_flip: function(block, card_value) {
        ...
        cc.audioEngine.stopMusic(false);
        this.scheduleOnce(function(){
            var url = cc.url.raw("resources/sounds/" + card_value + ".mp3");
            cc.audioEngine.playMusic(url, false);
        }, 0);
