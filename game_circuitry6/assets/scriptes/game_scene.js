...
cc.Class({
    ...
    on_start_game: function() {
        ...
        for(var index = 0; index < 15; index ++) {
            if(this.game_map[index] === -1 || this.game_map[index] === 9 || this.game_map[index] === 11) {
                continue;
            }
            var has_type = false;
            for(var j = 0; j < type_array.length; j ++) {
                if(type_array[j] === this.game_map[index]) {
                    has_type = true;
                    break;
                }
            }
            if(has_type === false) {
                type_array.push(this.game_map[index]);
            }
        }
        ...
        for(var i = 0; i < type_array.length; i ++) {
            this.place_drag_type(type_array[i], xpos, ypos);
            xpos += width;    
        }
    },
