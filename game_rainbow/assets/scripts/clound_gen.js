cc.Class({
    ...
    add_recv_count: function() {
        this.succes_time ++;
        if(this.succes_time < this.recv_set.length) {
            this.recv_set[this.succes_time].active = true;    
        }
        this.brige_state_com.show_state(this.succes_time);
        if(this.succes_time >= 7) {
            this.caihong_star.stopSystem();
            this.caihong_star.resetSystem();
            cc.audioEngine.stopMusic(false);
            var url = cc.url.raw("resources/sounds/rain_bom_ok.mp3");
    ...
    gen_new_clound: function(type, pos) {
        ...
        cc.audioEngine.stopMusic(false);
        var url = cc.url.raw("resources/sounds/clound_gen.mp3");
        cc.audioEngine.playMusic(url, false);
    ...
    onLoad: function () {
        ...
        this.brige_state_com = cc.find("UI_ROOT/anchor-center/jishui_root/qiao_root").getComponent("bridge_state");
    
        this.caihong_star = cc.find("UI_ROOT/anchor-center/jishui_root/caihong_star").getComponent(cc.ParticleSystem);
    ...
    start_game: function() {
        for(var i = 1; i < this.recv_set.length; i ++) {
            var rev_com = this.recv_set[i].getComponent("recv_rain");
            rev_com.reset_water();
            this.recv_set[i].active = false;
        }
        this.recv_set[1].active = true;
        this.succes_time = 1; // 第一个是成功的
        this.brige_state_com.show_state(this.succes_time);
