...
cc.Class({
    ...
    change_lever2: function() {
        ...
        if(this.level_2_1_ok === false && this.hit_set[0][3] !== null && this.hit_set[1][3] !== null && this.hit_set[2][3] !== null) { // 改变第一根杠杆
            this.level_2_1_ok = true;
            var move_time = 3;
            var up1_com = this.front_car_part.getChildByName("up1").getComponent("up_and_down_move");
            up1_com.move_by(move_time, 86);
            
            var rot = cc.rotateBy(3, 360);
            this.hit_set[0][3].runAction(rot);
            this.hit_set[1][3].runAction(rot.clone());
            this.hit_set[2][3].runAction(rot.clone());
        }
        
        if (this.level_2_2_ok === false && this.hit_set[3][3] !== null && this.hit_set[4][3] !== null && this.hit_set[5][3] !== null) { // 改变第二根杠杆
            this.level_2_2_ok = true;
            var move_time = 3;
            var down2_com = this.front_car_part.getChildByName("down2").getComponent("up_and_down_move");
            down2_com.move_by(move_time, -169);
            
            var rot = cc.rotateBy(3, 360);
            this.hit_set[3][3].runAction(rot);
            this.hit_set[4][3].runAction(rot.clone());
            this.hit_set[5][3].runAction(rot.clone());
        }
    }, 
    ...
    change_lever3: function() {
        ...
        if(this.level_3_1_ok === false && this.hit_set[0][3] !== null && this.hit_set[1][3] !== null) { // 改变第一根杠杆
            ...
            down2_com.move_by(move_time, 48);
            this.hit_set[0][3].runAction(cc.rotateBy(3, 360));
            this.hit_set[1][3].runAction(cc.rotateBy(3, 360));
            this.call_latter(function(){
                this.hit_set[0][3].stopAllActions();
                this.hit_set[1][3].stopAllActions();
            }.bind(this), move_time);
        }
        
        if (this.level_3_2_ok === false && this.hit_set[2][3] !== null && this.hit_set[3][3] !== null && this.hit_set[4][3] !== null) { // 改变第二根杠杆
            this.level_3_2_ok = true;
            /*var huo_sai2 = this.back_car_part.getChildByName("huosai1");
            huo_sai2.active = true;
            
            
            if(this.level_3_1_ok === false) {
                if(this.hit_set[0][3]) {
                    this.hit_set[0][3].active = true;
                }
                if(this.hit_set[1][3]) {
            ...
            var work_time = 6;
            this.call_latter(function(){
                huo_sai2.getComponent(sp.Skeleton).clearTracks();
            }.bind(this), work_time);*/
        }
    }, 
        ...
        if (this.game_mode >= 1 && this.now_tip_main_type != main_type) {
            ...
            for(index = 0; index < this.hit_set.length; index++) {
                if(this.hit_set[index][0] == main_type) { // 这里绘制提示
                    ...
                    var pos = this.tip_type_root.convertToNodeSpaceAR(world_pos);
                    new_node.x = pos.x;
                    new_node.y = pos.y;
    ...
    enter_car_front_mode: function() {
        ...
        var main_car = cc.instantiate(this.decro_prfabs[1].sub_set[0]);
        this.car_parts.addChild(main_car);
        var main_car_com = main_car.getComponent("decorative_part");
        main_car_com.set_start_pos(xpos, ypos);
        xpos = xpos - delta;
    ...
    play_pass_level1: function() {
        ...
        var men = this.main_car_part.getChildByName("men");
       
        var time = 2;
        for(var i = 0; i < this.hit_set.length; i ++) {
            this.hit_set[i][3].runAction(cc.rotateBy(time, -90));   
        }
        men.runAction(cc.rotateBy(time, -90));
        // ci_lun.runAction(cc.rotateBy(time, -90));
        
        this.call_latter(function(){
            machine_man.runAction(cc.moveBy(3, 760, 0));
        }.bind(this), time)
    ...
    play_pass_level2: function() {
        ...
        var seq = cc.sequence([cc.delayTime(3), cc.moveBy(3, 870, 0)]);
    ...
    play_pass_level3: function() {
        var anim_root_com = this.back_car_part.getChildByName("qiqiu_root").getComponent("level3_anim");
        anim_root_com.play_anim(this.hit_set);
        ...
        var seq = cc.sequence([cc.delayTime(13), func7]);
