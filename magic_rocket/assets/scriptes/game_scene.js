...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.fire_root = cc.find("UI_ROOT/anchor-center/car_root/fire_root");
        this.fire_root.active = false;
        ...
        this.magic_car_wheel = this.magic_car_body;
        ...
        this.pass_trunk = cc.find("UI_ROOT/anchor-center/pass_truk");
    ...
    on_start_game: function() {
        ...
        // 天台下来
        {
            var top_left = cc.find("UI_ROOT/anchor-top/top/top_left");
            top_left.x = 0;
            top_left.y = 0;
            var top_right = cc.find("UI_ROOT/anchor-top/top/top_right");
            top_right.x = 0;
            top_right.y = 0;
            
            var top = cc.find("UI_ROOT/anchor-top/top");
            top.y = 160;
            top.getComponent("move_action").play();
        }
        // end 
        ...
        this.pass_trunk.getComponent("move_action").play();
        this.pass_trunk.active = false;
    ...
    hit_car_part: function(decorative_com, w_pos, main_type, sub_type) {
        ...
        if (this.game_mode >= 5 && this.now_tip_main_type != main_type) {
            ...
            for(index = 0; index < this.hit_set.length; index++) {
                if(this.hit_set[index][0] == main_type) { // 这里绘制提示
                    ...
                    if (flip_x !== 0) {
                        new_node.scaleX = -new_node.scaleX;
                    }
        ...
        // 绘制轮胎提示
        if (this.game_mode === 4 && this.now_tip_main_type != main_type) {
            this.tip_type_root.removeAllChildren();
            this.now_tip_main_type = main_type;
            for(index = 0; index < this.hit_set.length; index++) {
                if(this.hit_set[index][0] == main_type) { // 这里绘制提示
        ...
        for(index = 0; index < this.hit_set.length; index ++) {
            ...
            if (flip_x !== 0) { // 全部转换成0.5, 0.5锚点的世界坐标
                w_pos = cc.p(w_pos.x, w_pos.y);
                w_pos.x -= decorative_com.node.getContentSize().width;
            }
    ...
    enter_main_car_model: function() {
        ...
        for(var i = 0; i < this.main_car_prefabs.length; i ++) {
            ...
            var size = main_car.getContentSize();
            var w = size.width * 0.5 * main_car.scale;
            var h = size.height * 0.5 * main_car.scale;
            var dx = (main_car.anchorX - 0.5) * w;
            var dy = (main_car.anchorY - 0.5) * h;
            main_car_com.set_start_pos(xpos, ypos + dy);
            xpos = xpos + delta;        
            main_car.active = true;
        }
    ...
    // [main_type, sub_type, w_pos, now_object, flip_x]
    computer_hit_pos: function(now_main_type) {
        ...
        for(index = 0; index < front_car_com.decorative_item.length; index ++) {
            ...
            var sub_type = 0;
            var flip_x = front_car_com.decorative_item[index].flip_x;
            ...
            var a_set = [main_type, sub_type, w_pos, null, flip_x];
        ...
        for(index = 0; index < back_car_com.decorative_item.length; index ++) {
            ...
            var sub_type = 0;
            var flip_x = back_car_com.decorative_item[index].flip_x;
        ...
        for(index = 0; index < main_car_com.decorative_item.length; index ++) {
            ...
            var sub_type = 0;
            var flip_x = main_car_com.decorative_item[index].flip_x;
    ...
    get_all_decor_parts_num: function() {
        ...
        // 飞机有两个
        num = num + this.decro_prfabs[0].sub_set.length;
        return num;
    }, 
    
    on_pass_truk_back_and_again_game: function() {
        var delay = cc.delayTime(1);
        var m = cc.moveTo(2.1, 2000, -40);
        ...
        var seq = cc.sequence([delay, m, call]);
        this.pass_trunk.runAction(seq);
    }, 
    ...
    on_out_factory_click: function() {
        ...
        this.fire_root.active = true;
        body_actions.push(cc.moveTo(0.05, 0, 0));
        body_actions.push(cc.scaleTo(0.1, 1, 0.9));
        body_actions.push(cc.scaleTo(0.1, 1, 1));
        var c1 = cc.callFunc(function() {
            // 拖车回去,游戏重新开始
            // this.on_pass_truk_back_and_again_game();
            // end
            var mby = cc.moveBy(1.5, 0, 1920);
            this.magic_car.runAction(mby);
        }.bind(this), this);
        ...
        // 天台打开
        var top_left = cc.find("UI_ROOT/anchor-top/top/top_left");
        var comp = top_left.getComponent("move_action");
        comp.play();
        var top_right = cc.find("UI_ROOT/anchor-top/top/top_right");
        comp = top_right.getComponent("move_action");
        comp.play();
        // end 
    ...
    on_next_game_mode: function() {
        ...
        if(this.game_mode == 2) { // 进入车头模式
            ...
            this.enter_whell_mode();
        }
        else if(this.game_mode == 5) { // 进入装装饰模式
