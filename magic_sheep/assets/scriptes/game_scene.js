...
cc.Class({
    ...
    enter_car_front_mode: function() {
        ...
        for(var i = 0; i < this.front_car_prefabs.length; i ++) {
            ...
            front_car_com.set_start_pos(xpos, ypos - 20);
    ...
    on_out_factory_click: function() {
        ...
        // 水
        var up_water = cc.find("UI_ROOT/anchor-center/up_water");
        var fin = cc.fadeTo(2, 255);
        up_water.runAction(fin);
        var down_water = cc.find("UI_ROOT/anchor-center/down_water/wator_root");
        down_water.runAction(fin.clone());
        
        
        var m = cc.moveBy(0.5, 0, 30);
        down_water.runAction(m);
        up_water.runAction(m.clone());
        // end 
        // 零件拖走
        var move = cc.moveBy(1.6, -1920, 0);
        this.car_parts.runAction(move);
        ...
        this.call_latter(function() {
            ...
            up_water.runAction(fout);
            down_water.runAction(fout.clone());
            var m = cc.moveBy(0.5, 0, -30);
            down_water.runAction(m);
            up_water.runAction(m.clone());
            // end 
    ...
    enter_decor_mode: function() {
        ...
        var num = this.get_all_decor_parts_num_more_than(1);
        console.log("" + num);
        ...
        for (var j = 1; j < this.decro_prfabs.length; j ++) {
            for(var i = 0; i < this.decro_prfabs[j].sub_set.length; i ++) {
                ...
                decor_com.set_start_pos(xpos + dx, ypos + dy - 30);
                xpos = xpos + delta;
            }
        }
    ...
    enter_whell_mode: function() {
        ...
        var start_x = (this.game_mode) * 1920; 
        var num = this.decro_prfabs[0].sub_set.length + 1;
        ...
        {
            for(var i = 0; i < this.decro_prfabs[0].sub_set.length; i ++) {
                var wheel = cc.instantiate(this.decro_prfabs[0].sub_set[i]);
                wheel.active = true;
                this.car_parts.addChild(wheel);
                var wheel_com = wheel.getComponent("decorative_part");
                wheel_com.set_start_pos(xpos, ypos - 100);
                wheel_com.dst_pos = 0;
                xpos = xpos + delta;
            }
        }
