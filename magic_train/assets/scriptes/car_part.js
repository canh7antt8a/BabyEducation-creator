...
cc.Class({
    ...
    play_wheel_anim: function() {
        var rot = cc.rotateBy(0.7, 360);
        var f = cc.repeatForever(rot.clone());
        for(var i = 0; i < this.wheel_num; i ++) {
            var rot = Math.random() * 360;
            var wheel = this.node.getChildByName("tw" + (i + 1));
            // wheel.rotation = rot;
            wheel.runAction(f.clone());
        }
    }
